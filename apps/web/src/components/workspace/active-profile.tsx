...
export function useActiveTeamId(): Id<"teams"> | undefined {
  const profileFromContext = use(ActiveProfileContext);
  const profileIdFromRoute = useActiveProfileId();
  const skipQuery =
    profileFromContext !== null || profileIdFromRoute === undefined;
  const profiles = useQuery(api.profiles.list, skipQuery ? "skip" : {});
  if (profileFromContext !== null) return profileFromContext.teamId;
  if (profileIdFromRoute === undefined) return undefined;
  return profiles?.find((p) => p._id === profileIdFromRoute)?.teamId;
}
