...
function normalizeRole(raw: string): string {
  ...
  if (r === "assistant" || r === "deepseek" || r === "ai" || r === "bot")
    return "Assistant";
...
function conversationStableId(
  ...
  if (isString(u) && u.length > 0) return `deepseek-${u}-${String(index)}`;
  if (isNumber(u)) return `deepseek-${String(u)}-${String(index)}`;
  return `deepseek-row-${String(index)}`;
}
...
export function parseDeepSeekExportBuffer(
  ...
  if (fromZip) {
    return parseDeepSeekExportJsonText(fromZip);
  }
  return parseDeepSeekExportJsonText(textFromUtf8(bytes));
}
