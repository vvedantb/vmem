...
export interface RelatedSymbol {
  ...
  path: string;
  kind: CodeNodeKind;
  /** Canonical relationship label for the panel — one of imports/imported_by/
   *  calls/called_by/contains/extends/implements. */
  direction:
    | "imports"
    | "imported_by"
    | "calls"
...
/**
 * Get symbols related to `symbolId` via any edge type, with the side of the
 * relationship encoded so the panel can group "imports" vs "imported_by".
 */
export function getRelatedSymbols(
  ...
  // Outgoing direction labels
  const out: Record<CodeEdge["type"], RelatedSymbol["direction"]> = {
    imports: "imports",
    calls: "calls",
    contains: "contains",
    has_method: "contains",
  ...
  // Incoming direction labels
  const incoming: Record<CodeEdge["type"], RelatedSymbol["direction"]> = {
    imports: "imported_by",
    calls: "called_by",
    contains: "contained_by",
    has_method: "contained_by",
    extends: "extended_by",
    implements: "implemented_by",
    starts_process: "in_process",
    includes: "starts_process",
  };

  for (const edge of apiEdges) {
    if (edge.fromId === symbolId) {
      const other = byId.get(edge.toId);
      if (!other) continue;
      const key = `${edge.type}:${other.id}`;
    ...
    } else if (edge.toId === symbolId) {
      const other = byId.get(edge.fromId);
      if (!other) continue;
      const key = `${edge.type}:in:${other.id}`;
      if (seen.has(key)) continue;
      seen.add(key);
      related.push({
        id: other.id,
