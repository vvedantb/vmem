...
function conversationStableId(conv: Conversation, index: number): string {
  ...
  if (typeof stamp === "number" || (stamp && stamp.length > 0)) {
    return `claude-${String(stamp)}-${String(index)}`;
  }
...
export function parseClaudeExportJsonText(jsonText: string): ParseExportResult {
  ...
  if (rows.length === 0) {
    return { ok: false, error: NO_MESSAGES_ERROR };
  }
  return { ok: true, rows };
}
...
export function parseClaudeExportBuffer(
  ...
  return parseClaudeExportJsonText(jsonText);
}
