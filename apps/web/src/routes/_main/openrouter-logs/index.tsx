...
import { LogsSummary } from "./_components/LogsSummary";
import { LogsFiltersDropdown } from "./_components/LogsFiltersDropdown";
import { LogsTable } from "./_components/LogsTable";

export const Route = createFileRoute("/_main/openrouter-logs/")({
  component: OpenRouterLogsPage,
});

const PAGE_SIZE = 50;

/**
 * `/openrouter-logs` — observability dashboard for every OpenRouter call
 * vmem fires (chat completions + embeddings).
 *
 * Per CLAUDE.md the route is a thin orchestrator: data fetching + nuqs
 * filter state lives here, presentation lives in `_components/`. Convex's
 * live query subscriptions mean a fresh log row appears at the top of
 * the table within ~1s of being inserted by the wrapper.
 */
function OpenRouterLogsPage() {
  const [params, setParams] = useQueryStates(openRouterLogsSearchParams);
  ...
  // Server-paginated query: scope + filters become args, every change
  // triggers a fresh paginator. We build the args object lazily so a
  // missing teamId in `team` scope doesn't blow up — `usePaginatedQuery`
  // accepts `"skip"` to short-circuit.
  ...
  // Sort direction is purely presentational (server already returns desc);
  // for the rare ascending case we reverse client-side. The current page
  // size is small enough that the cost is negligible.
