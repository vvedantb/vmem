...
4. Write a commit message following the repo's convention (lowercase prefix like `feat:`, `fix:`, `refactor:`, etc.) — title summarizes WHY, not WHAT. Add a body (separated by blank line) explaining the reasoning/context in 1-3 short lines.
5. Commit using a HEREDOC (title + blank line + body) for the message (do NOT use `git add` before committing)
