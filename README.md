...
Memories live in Convex. Convex also handles auth, profiles, teams, the web/API surface, and scheduled work. The web app and Chrome extension are clients on top of that. Retrieval ranks memories with Convex full-text search, lexical/synonym overlap, recency, temporal windows, and optional vector similarity when an OpenRouter embedding key is configured. When `TYPESAFE_API_KEY` is set, TypeSafe Jev judges the top 20 hits (keep / score / best); missing key leaves hybrid ranking as-is. Pass `judge: "off"` to skip Jev for ablation. List and retrieve honor `type`, `tags`, and `status` filters (tags are normalized to lowercase-hyphenated). Agentic instruction store/update requires `OPENROUTER_API_KEY` and returns HTTP 422 `openrouter_required` without it. `summarize: true` joins ranked titles and does not call an LLM. Each hit still explains itself in a Context Trace.
