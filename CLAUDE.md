...
Slides live-share (`/slides` deck — "follow the presenter", Teams-style):

- Two new tables (`presentationSessions` + `presentationParticipants`, fields in `validators.ts`); all functions in `convex/presentations.ts` are **public** (`mutation`/`query`, NOT `authMutation`) because `/slides` is reachable without sign-in. `createSession` attaches `hostUserId` opportunistically (reads `getUserIdentity()`, never requires it).
- **Single driver, no take-control**: only the browser holding the secret `hostKey` (returned once from `createSession`, kept in `localStorage` under `vmem:presentation-host:<code>`) can `setSlide`/`stopSharing`. `getSession` NEVER returns `hostKey`. Viewers either follow `slide` live or detach to private browsing (`mode` state in `usePresentationSync`), then "Back to live". The **slide is the only synced state** — build steps replay locally on each client (timer-driven, self-converging), so never sync `step`.
- Presence = lightweight heartbeat (NOT `@convex-dev/presence`): `heartbeat` upserts one `presentationParticipants` row per `participantKey` every 15s; `listParticipants` returns rows seen <30s and is subscribed by the **presenter only** (viewers don't, to bound fan-out). The presenter heartbeats with role `host` (excluded from the viewer count). Stale rows drop within one heartbeat cycle. Daily `pruneStaleInternal` cron (05:00 UTC) GCs ended/idle-24h sessions + their rows.
- Web: route stays `/slides` with `?session=<code>` added to the search schema. `usePresentationSync.ts` owns identity/mode/subscriptions/heartbeat and returns `effectiveSlide` + `onNavigate` for the (otherwise unchanged) `SlideDeck`. `SlideDeck` now derives transition `direction` from a prev-slide ref delta so follower-driven changes animate correctly. Overlay chrome: `PresentationControls` (auto-hides ~3s; wrapper is click-through so deck click-nav still works) + `PresentationHostBar` (share link copy + viewer list + stop) + `JoinNamePrompt` (anonymous viewer names themselves on join).
