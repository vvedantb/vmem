...
- **Background token refresh runs IN the service worker** (`refreshConvexTokenFromClerk` called directly from `auth.ts`) — the SW has `chrome.cookies` + host permissions, so Clerk's syncHost bridge can read `__clerk_db_jwt`/`__client` there. Never route this through an offscreen document: offscreen contexts only get `chrome.runtime`, Clerk's cookie read silently fails, and every post-restart sync skips "no-session" forever until the popup is opened (root cause of day-long silent gaps; offscreen layer deleted 2026-06-11).
- **`@clerk/ui/no-rhc` is aliased to a stub in the background build** (`createBackgroundConfig` → `src/background/clerk-ui-stub.ts`): the non-background Clerk branch dynamic-imports it, `inlineDynamicImports` inlines it, and its module body touches `document` at eval — an uncaught ReferenceError on every SW boot that can get the worker blocklisted for the session (no events fire at all). Also drops the SW bundle 3.2 MB → ~1 MB.
- Extension unit tests need `--test-force-exit`: the `@clerk/chrome-extension` import graph (reachable from `sync-scheduler` → `auth`) leaves live handles after tests pass. The chrome mock needs `runtime.id` (webextension-polyfill throws at import without it) and NO `cookies` key (the refresh guard then short-circuits to a null token — hermetic, no network).
