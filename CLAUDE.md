...
- `usePresentationDeck()` context (`_components/PresentationDeckContext.tsx`, provided by `slides.tsx`) feeds `{ sessionCode, participantKey, hostKey }` to slide components, which `SlideDeck` renders with no props. `usePresentationSync.ts` returns `effectiveSlide` + `onNavigate` for the (unchanged) `SlideDeck`, which derives transition `direction` from a prev-slide ref delta so follower-driven changes animate correctly. Overlay chrome: `PresentationControls` (auto-hides ~3s; wrapper is click-through so deck click-nav still works) + `PresentationHostBar` (share link copy + stop). Daily `pruneStaleInternal` cron (05:00 UTC) GCs ended/idle-24h sessions + their votes.
