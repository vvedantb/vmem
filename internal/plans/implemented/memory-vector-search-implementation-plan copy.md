...
- No SDK — raw fetch (simple POST, avoid deps)
...
**Modify:** `packages/backend/src/neo4j/setup.ts` (after line 42)
...
Lines 14-27 (`MemoryNode` interface):

```ts
embedding: number[] | null;  // NEW field
```

Lines 59-63 (`ScoreBreakdown` interface):

```ts
vector: number; // NEW field
```
...
**Modify:** `packages/backend/src/neo4j/memoryService.ts` → `createMemory()` (lines 241-336)
...
**Modify:** `packages/backend/src/neo4j/memoryService.ts` → `retrieveMemories()` (lines 665-736)
