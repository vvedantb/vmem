...
- **Three dashboard hot-paths cached with 30s TTL**: `getStatsInternal`, `getRecentActivityInternal`, and `getGraphDataInternal` are now wrapped via `ActionCache.fetch` in `dashboardApi.ts` and `graphApi.ts`. Versioned cache names (`-v1`) let us bump the key if the action's return shape ever changes
