...
| `search(query, options?)` | Hybrid retrieve; `type`/`tags`/`status` filters; `summarize: true` joins titles; Jev rerank when the deployment has `TYPESAFE_API_KEY` (`judge: "off"` skips) |
