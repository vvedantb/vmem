...
Labelled retrieve eval (IR, no LLM judge): `pnpm --filter @vmem/backend eval:bench`. Hybrid vs TypeSafe Jev on that same harness: `EVAL_JEV=1 pnpm --filter @vmem/backend eval:jev` (needs `TYPESAFE_API_KEY`; does not mock). SuperMemory / Mem0 gap analysis and Convex-only roadmap: `tests/memory/competitive-brief.md`. TypeSafe Jev retrieve-gate: `tests/memory/jev-retrieve-gate.md`.
