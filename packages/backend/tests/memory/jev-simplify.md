...
In-process eval (`eval/retrieve.ts`) never calls System One — labelled IR stays hybrid-only. Live HTTP/MCP/dashboard retrieve is where Jev runs.
...
- 422 when the TypeSafe key is absent.
