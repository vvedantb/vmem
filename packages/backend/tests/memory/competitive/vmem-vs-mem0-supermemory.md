...
| **vmem default Jev-rerank** | Same hybrid, then AI Gateway `typesafe-ai/jev` on the top 20 (**rerank only**, no noul hard-drop — #187) | Same                                                                                                                                        | Requires `AI_GATEWAY_API_KEY`. Missing key → row stays `—` (does **not** copy hybrid numbers)                                                    |
...
| vmem default Jev-rerank (no hard-drop, #187) | —        | —        | —          | —         | —         | —         | —      | —      | **TODO: `AI_GATEWAY_API_KEY`**. Pre-#187 hard-drop R@5 **84.6%** is superseded and is **not** the product default             |
...
### Optional vmem Jev column (`AI_GATEWAY_API_KEY`)

Same `AI_GATEWAY_API_KEY` as product retrieve (model `typesafe-ai/jev`). Without it the Jev row stays `—`.
...
- **vmem production retrieve** is hybrid + Jev rerank when `AI_GATEWAY_API_KEY` is set (#183/#187). CI `eval:bench` is hybrid-only. Do not treat pre-#187 hard-drop Jev (R@5 84.6%, 16 gold titles removed) as current default.
