...
| --------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **vmem hybrid-only**        | In-process Convex ranker (`judge: "off"`), same as `eval:bench` full hybrid                                  | Local corpus rows (no HTTP)                                                                                                                 | Hybrid FTS + vector + graph + temporal. Synthetic embeddings unless `OPENROUTER_API_KEY` (`openai/text-embedding-3-small`)                       |
...
- **vmem** numbers in git from #184/#186 used **deterministic synthetic embeddings** (no OpenRouter). Vector quality is not `text-embedding-3-small`. Re-run with `OPENROUTER_API_KEY` before quoting vmem externally.
