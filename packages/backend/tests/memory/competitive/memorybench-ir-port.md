...
`LOCOMO_IR_JUDGE=jev` needs `AI_GATEWAY_API_KEY` to call Jev (`typesafe-ai/jev`). Fail-open matches prod retrieve: missing key or a Jev error keeps the hybrid ranking (no 422, no mock numbers). That is **not** `EVAL_JEV`’s fail-closed labelled comparison. `pnpm test` keeps the mapping + fixture retrieve tests **offline**. No `OPENAI_API_KEY` / `MEM0_API_KEY` / `SUPERMEMORY_API_KEY`. Do not commit locomo10 metrics or secrets.
...
| LoCoMo-IR + Jev (`LOCOMO_IR_JUDGE=jev`) | 1 Jev evaluate / query (retrieve rerank only) | AI Gateway `typesafe-ai/jev`, not gpt-4o answer+judge                                          | `AI_GATEWAY_API_KEY` (fail-open without it)     | same IR metrics after prod-like Jev rerank              |
| LoCoMo-IR + AI Gateway embeds           | 0 chat                                        | ~cents (`text-embedding-3-small` on ~0.5–6k texts)                                             | `AI_GATEWAY_API_KEY` optional                   | same IR, better vectors                                 |
